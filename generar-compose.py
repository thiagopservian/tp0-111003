...
def generate_compose(output_file, client_amount):
    ...
    for i in range(1, client_amount + 1):
        ...
        lines.append(f"      - CLI_NOMBRE=Nombre{i}")
        lines.append(f"      - CLI_APELLIDO=Apellido{i}")
        lines.append(f"      - CLI_DOCUMENTO=3090446{i}")
        lines.append(f"      - CLI_NACIMIENTO=1999-03-17")
        lines.append(f"      - CLI_NUMERO=7574")
